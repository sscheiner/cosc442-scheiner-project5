...
public class WMethod{
  ...
  static List<String> unitTests = new ArrayList<String>();
   ...
   */
   ...
   /**
    * task 3 test generation method
    * @param tests, the list of test cases generated by the WMethod tool
    */
   private static void generateJUnitTests(String test, String expectedOutput, int counter) {
   
	   //write tests in file due to errors
	try(FileWriter fw = new FileWriter("JunitTests.txt", true);
		BufferedWriter bw = new BufferedWriter(fw);
		PrintWriter out = new PrintWriter(bw))
			{
				   if(expectedOutput.contains("yes")){
					   
					   out.println(
							   "@Test\r\n"
							+	"public void testCase" + counter + "(){"
					   		+ "\r\n"
					   		+ "assertTrue(JamesBond.bondRegex(\"" + test + "\"));"
					   		+ "\r\n"
					   		+ "}");
			   }
			   
				   else if(expectedOutput.contains("no")){
					   
					   out.println(  "@Test\r\n" 
							+	"public void testCase" + counter + "(){"
					   		+ "\r\n"
					   		+ "assertFalse(JamesBond.bondRegex(\"" + test + "\"));"
					   		+ "\r\n"
					   		+ "}");
			   }
			} catch (IOException e) {
   ...
   private static void runTests(Vector<String> tests) {
	   ...
	   for(String x : tests) {
		   //intersperse input string with space seperation
		   String seperator = " ";
		   String seperatedInput = "";
		   for(int i = 0; i < x.length(); i++) {
			   seperatedInput += (x.charAt(i) + seperator);
		   }
		   
		   String expectedOutput = Utilities.runFSM(FSM, 1, seperatedInput, " ");
		   generateJUnitTests(x, expectedOutput, testCounter);
		   testCounter++;
	   }
   }
